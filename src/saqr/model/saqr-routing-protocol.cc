...
#include "saqr-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("SaqrRoutingProtocol");

namespace saqr {

NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

const uint32_t RoutingProtocol::SAQR_PORT = 5678;

// Set metadata
TypeId RoutingProtocol::GetTypeId(void)
{
  static TypeId tid = TypeId("ns3::saqr::RoutingProtocol")
                                            ...
                                            Socket::SocketErrno &sockerr)
{
  ...
  if (m_socketAddresses.empty ())
  {
    ...
    NS_LOG_LOGIC ("No saqr interfaces");
    Ptr<Ipv4Route> route;
    return route;
  }
                                 ...
                                 ErrorCallback ecb)
{
  ...
  // comment it since SAQR could bear this kind of loop
  /*
...
void RoutingProtocol::NotifyInterfaceUp(uint32_t interface)
{
  ...
  {
    NS_LOG_WARN("SAQR does not work with more than one address per each interface.");
  }
   ...
   */
  socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvSaqr, this));

  socket->BindToNetDevice(l3->GetNetDevice(interface));  
  socket->Bind(InetSocketAddress(iface.GetLocal (), SAQR_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvSaqr, this));
  socket->BindToNetDevice (l3->GetNetDevice (interface));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), SAQR_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
...
void RoutingProtocol::NotifyAddAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
  ...
  {
    NS_LOG_LOGIC("SAQR does not work with more than one address per each interface. Ignore added address");
  }
}
...
void RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if(socket)
  {
    ...
    if(m_socketAddresses.empty())
    {
      NS_LOG_LOGIC("No saqr interface");
      m_nb.Clear();
      m_routingTable.Clear ();
      return;
    }
  ...
  {
    NS_LOG_LOGIC("Remove address not participating in SAQR operation");
  }
}
...
Ptr<Ipv4Route> RoutingProtocol::LoopbackRoute(const Ipv4Header &hdr, Ptr<NetDevice> oif) const
{
  ...
  NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid SAQR source address not found");
...
void RoutingProtocol::RecvSaqr (Ptr<Socket> socket)
{
  ...
  NS_LOG_DEBUG ("SAQR node " << this << " received a SAQR packet from " << sender << " to " << receiver);

  TypeHeader tHeader (SAQRTYPE_HELLO);
  packet->RemoveHeader (tHeader);
  if (!tHeader.IsValid ())
  {
    NS_LOG_DEBUG ("SAQR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
    return;
  }
  switch (tHeader.Get ())
  {
...
RoutingProtocol::SendTo (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination)
{
  ...
  socket->SendTo (packet, 0, InetSocketAddress (destination, SAQR_PORT));
}
